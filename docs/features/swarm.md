# Swarm: Agent Teams With a File-Backed Mailbox (Design)

Status: proposed (design)

This document proposes a "Swarm" feature for Roboppi: coordinated teams of
...
- Distributed (multi-host) swarms.
- Nested swarms (teammates spawning their own teams).
...
Swarm introduces three cooperating pieces:

1. **Swarm Coordinator (Runner-owned)**
...
2. **Swarm Store (file-backed, deterministic)**
...
3. **Swarm Tools (model-facing)**
   - A constrained interface that agents use instead of editing mailbox files.
   - Exposed via:
     - a Roboppi CLI subcommand (`roboppi swarm ...`), callable via a restricted
       `Bash(roboppi swarm:*)` tool in Claude Code
...
Swarm is primarily policy-layer orchestration, but it needs a mechanism-grade
mailbox with correctness under concurrency. We therefore treat the Swarm Store +
...
- **Recommended**: Swarm requires `--supervised` (CoreIpcStepRunner) so the
  Coordinator can cancel/timeout teammates safely and stream events into the TUI.
- **Fallback**: In unsupervised mode, Swarm still works, but safety and
...
Swarm resources are stored under the workflow `context_dir` so that:
...
`_swarm/mailbox/_events.jsonl` (metadata only by default):
...
- periodic timer in the Swarm Coordinator
- explicit CLI: `roboppi swarm housekeep --context <dir>`

---

## 6. Model-Facing Tooling

Agents should not manipulate `_swarm/` files directly. Instead, Roboppi exposes
Swarm operations as a tool.

### 6.1 CLI tool surface (v1)

Add a `roboppi swarm` CLI group:

- `roboppi swarm init --context <dir> --team <name>`
- `roboppi swarm members list --context <dir>`
- `roboppi swarm message send --context <dir> --from <memberId> --to <memberId> --topic <t> --body <text>`
- `roboppi swarm message broadcast --context <dir> --from <memberId> --topic <t> --body <text>`
- `roboppi swarm message recv --context <dir> --for <memberId> [--claim] [--max N] [--wait-ms M]`
- `roboppi swarm message ack --context <dir> --for <memberId> --message-id <uuid>`

- `roboppi swarm tasks add --context <dir> --title ... --description ... [--depends-on ...]`
- `roboppi swarm tasks list --context <dir> [--status pending|in_progress|completed]`
- `roboppi swarm tasks claim --context <dir> --task-id <uuid> --member <memberId>`
- `roboppi swarm tasks complete --context <dir> --task-id <uuid> --member <memberId> [--artifact <path>]`
...
```
roboppi swarm message ack --context <dir> --for lead --claim-token <opaque>
```
...
- In `src/worker/adapters/claude-code-adapter.ts`, map `MAILBOX` to
  `Bash(roboppi swarm:*)` (or `Bash(roboppi mailbox:*)`)
...
- Prefer `roboppi swarm message ...` subcommands that accept structured JSON via
...
`roboppi swarm mcp` that exposes tools:
...
The Coordinator creates `_swarm/` directories and writes `team.json` +
...
- `ROBOPPI_SWARM_CONTEXT_DIR=<context_dir>`
- `ROBOPPI_SWARM_TEAM_ID=<team_id>`
- `ROBOPPI_SWARM_MEMBER_ID=<member_id>`
- `ROBOPPI_SWARM_MEMBERS_FILE=<context_dir>/_swarm/members.json`
...
To end a swarm deterministically:
...
Swarm can be used in workflows in two ways.

### 8.1 Swarm as a top-level workflow feature

Add optional `swarm:` config to `WorkflowDefinition`:
...
### 8.2 Swarm as an explicit step

Add a new step type (discriminated union at some future refactor):

- `swarm:` step that runs the Coordinator as a step, and completes when tasks
  are all completed.

This keeps Swarm opt-in and contained.

### 8.3 Workspace isolation (recommended)

Two teammates editing the same files can conflict. To reduce this, Swarm SHOULD
support (optionally) creating a git worktree per teammate:

- `<context_dir>/_swarm/worktrees/<memberId>/`
...
Swarm MUST be observable through existing mechanisms:

- Append-only event logs:
  - `_swarm/mailbox/_events.jsonl`
  - `_swarm/tasks/_events.jsonl`
...
- Messages and tasks are stored on disk. By default, Swarm MUST treat message
...
- For Claude Code, prefer allowing only `roboppi swarm:*` rather than full bash.

### 10.4 Identity and spoofing (local trust model)

Swarm is designed for local, same-user processes. We assume cooperative agents,
not adversarial isolation. However, the tool SHOULD reduce accidental spoofing:

- default `--from` to `ROBOPPI_SWARM_MEMBER_ID`
...
3. Integration tests (supervised mode):
   - spawn two CUSTOM worker steps that message each other using `roboppi swarm`
...
- Swarm state is local; resumption/replay semantics are not guaranteed in v1.
