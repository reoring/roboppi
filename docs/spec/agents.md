# Agents v1 Conformance Specification (Gap Closure)

Status: implemented
Last updated: 2026-03-02

This document defines the conformance requirements needed for the current Agents
implementation to fully align with `docs/features/agents.md`.
...
- Workflow top-level `agents:` integration and TUI agents tab visibility.
...
`roboppi agents message recv` MUST support:
...
For every `roboppi agents` subcommand failure, including argument/usage errors:
...
All tool-facing path inputs MUST be constrained to Agents-safe boundaries.

Requirements:

- All filesystem mutations for agents data MUST remain inside
  `<context_dir>/_agents`.
...
- uses `roboppi agents ...` commands (not only direct store API calls)
- includes two `CUSTOM` workers exchanging messages/tasks
- verifies emitted `agent_*` exec events and/or `_agents/*/_events.jsonl`
  artifacts
...
It is acceptable in v1 that both map to the same restricted `roboppi agents:*`
...
- [x] 3.1 implemented and covered by unit/AT tests
  - `--wait-ms 0`, timeout, and arrival-during-wait AT coverage in `tests/at/agents-cli.test.ts`
- [x] 3.2 implemented and covered by lifecycle tests
  ...
  - Cleanup trash kept within `_agents/.trash/` (not `os.tmpdir()`)
  - `tests/unit/agents/coordinator.test.ts` (bounded timeout, abort-only-unsettled, cleanup policy, cleanup event)
- [x] 3.3 implemented and covered by housekeeping tests
  - `tests/unit/agents/housekeeping-tasks.test.ts` (stale detection, metadata rewrite, idempotency, concurrent safety)
- [x] 3.4 implemented and covered by CLI error/help tests
  - Unknown subcommand, missing flag, semantic failure AT coverage in `tests/at/agents-cli.test.ts`
- [x] 3.5 implemented and covered by path-safety tests
  - `assertAgentsRootSafe()` + `validateMemberIdPath()` applied to all mutating commands including `init`, `recv`, `ack`, `housekeep`
  - `init` validates `--lead` and all `--json-stdin` member IDs via `validateMemberIdPath()`
  - Symlink escape + traversal rejection AT in `tests/at/agents-cli.test.ts` (including `init` subcommand)
- [x] 3.6 implemented and verified in CI test targets
  - `tests/at/agents-supervised-custom.test.ts` uses CLI subprocess calls (not direct store API)

## 6. Compatibility

This specification preserves compatibility with the `version: "1"` agents
on-disk objects and with the design direction in `docs/features/agents.md`.
