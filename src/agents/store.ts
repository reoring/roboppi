/**
 * Agent Store — file-backed mailbox with maildir semantics.
 *
 * Mechanism-grade: atomic rename for deliver/claim/ack, tmp+rename for
 * writes.  See `docs/features/agents.md` §5.
 */
...
/**
 * Create `<context_dir>/_agents/` layout idempotently.
 * Writes `team.json` and `members.json`.
 */
export async function initAgentsContext(opts: InitAgentsOptions): Promise<{
  teamId: string;
}> {
...
// ---------------------------------------------------------------------------
// Membership mutation helpers (atomic write via tmp+rename)
// ---------------------------------------------------------------------------

/**
 * Atomically rewrite `_agents/members.json` with the given members list.
 * Preserves `version: "1"` and deterministic ordering by `member_id`.
 */
export async function writeMembersConfig(
  contextDir: string,
  members: MemberEntry[],
): Promise<void> {
  ...
  const tmpDir = resolve(root, "tmp");
  await atomicJsonWrite(tmpDir, membersJsonPath(contextDir), config);
}

/**
 * Upsert a member in `_agents/members.json` (create or update by `member_id`).
 */
export async function upsertMember(
  contextDir: string,
  entry: MemberEntry,
): Promise<void> {
  const existing = await readMembers(contextDir);
  const idx = existing.members.findIndex(
    (m) => m.member_id === entry.member_id,
  );
  if (idx >= 0) {
    existing.members[idx] = { ...existing.members[idx], ...entry };
  } else {
    existing.members.push(entry);
  }
  await writeMembersConfig(contextDir, existing.members);
}

/**
 * Remove a member from `_agents/members.json` by `member_id`.
 * Rejects removal of the lead member.
 */
export async function removeMember(
  ...
  if (team.lead_member_id === memberId) {
    throw new Error(`Cannot remove the lead member "${memberId}"`);
  }
  const existing = await readMembers(contextDir);
  const filtered = existing.members.filter((m) => m.member_id !== memberId);
  if (filtered.length === existing.members.length) {
    throw new Error(`Member "${memberId}" not found`);
  }
  await writeMembersConfig(contextDir, filtered);
}
...
/**
 * Find a claimed message in `processing/` by claim token.
 *
 * Used by higher-level operations that need access to the original message
 * metadata (e.g., reply-and-ack helpers).
 */
export async function getClaimedMessageByClaimToken(
  contextDir: string,
  memberId: string,
  claimToken: string,
): Promise<{ filename: string; message: AgentMessage } | null> {
  const processingDir = inboxProcessing(contextDir, memberId);
  let entries: string[];
  try {
    entries = await readdir(processingDir);
